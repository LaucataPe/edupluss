...
export const fetchActivities = createAsyncThunk('activities/fetchActivities', async () => {
	try {
		const { data } = await axios(`https://edupluss.onrender.com/activities`);
		return data
	} catch (error: any) {
...
export const getEmpresaActivities = createAsyncThunk('activities/getEmpresaActivities', async (id: number) => {
	try {
		const { data } = await axios(`https://edupluss.onrender.com/activities/${id}`);
		return data;
	} catch (error: any) {
		throw new Error(error.message);
	}
...
export const getActivitiesByRole = createAsyncThunk('activities/getActivitiesByRole', async (id: number) => {
	try {
		const { data } = await axios(`https://edupluss.onrender.com/activities/role/${id}`);
		return data
	} catch (error: any) {
		throw new Error(error.message);
	}
