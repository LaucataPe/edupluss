...
const PDFWorkerProfile: React.FC<{ workerProfile: Role }> = ({ workerProfile }) => {
    const formattedSalary = workerProfile.salary && parseFloat(workerProfile.salary.replace(/\D/g, '')).toLocaleString('es-CO');
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {workerProfile.softSkills && workerProfile.softSkills.map((skill, index) => (
          <Text key={index} style={styles.listItem}>
            - {skill}
          </Text>
        ))}
        ...
        <Text style={styles.sectionContent}>{workerProfile.schedule}
            {!workerProfile.schedule && `No se especifica`}
        </Text>

        <Text style={styles.sectionTitle}>Salario:</Text>
        <Text style={styles.sectionContent}>{`${formattedSalary} COP`}
        {!workerProfile.salary && `No se especifica`}
        </Text>

        <Text style={styles.sectionTitle}>Experiencia:</Text>
        <Text style={styles.sectionContent}>
            {workerProfile.experience && `${workerProfile.experience?.[0]} a ${workerProfile.experience?.[1]} años`}
            {!workerProfile.experience && `No se especifica`}
        </Text>


        <Text style={styles.sectionTitle}>Trabajo Remoto:</Text>
        <Text style={styles.sectionContent}>{workerProfile.remote ? 'Sí' : 'No'}</Text>
      </Page>
    </Document>
  );
};
