...
function SelectEmpresa() {
  ...
  const handleSelect = () => {
    ...
    return (
      <>
      <h1 className="text-center text-4xl p-3">Selecciona una empresa</h1><br />
      <div className="flex px-20  ">
        ...
        <Link to='/'><button disabled={idEmpresa.length === 0 ? true : false} onClick={handleSelect}
        className="py-3 px-4 cursor-pointer justify-center items-center rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 transition-all text-sm dark:focus:ring-offset-gray-800">Ingresar</button></Link> 
        <p>{error ? error : ''}</p>
      </div>
      </> 
    );
